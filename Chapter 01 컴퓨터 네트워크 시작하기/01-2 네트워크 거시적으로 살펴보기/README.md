...
### Q16. 호스트는 크게 서버와 클라이언트로 분류하는데 이들은 완전히 베타적인 개념인가?(O/X)
...
### Q17. 패킷 교환 방식에서는 데이터가 작은 패킷 단위로 나뉘어 전송되는데, 각 패킷이 서로 다른 경로를 통해 수신지까지 이동할 수 있다. 그렇다면, 모든 패킷이 전송된 순서대로 도착할까?(O/X)
