...
- 인터넷 계층은 TCP/IP 모델의 계층이며, OSI 7계층에는 포함되지 않는다.
...
**X**
- 캡슐화는 송신 과정에서 헤더 및 트레일러를 추가하는 과정이며, 수신 과정에서는 이를 제거하는 디캡슐화가 이루어진다.
...
- 참조 모델을 사용하는 이유는 네트워크 설계와 문제 해결을 용이하게 하기 위함이지, 모든 프로토콜이나 장비가 이를 사용하도록 강제하는 목적이 아니다.
...
<summary>정답</summary>
<h4>X</h4>
가장 낮은 단계인 물리 계층에 도달했을 때, 각 계층에서 붙은 여러개의 헤더 내용이 캡슐화 돼서 데이터에 포함된 형태라고 볼 수 있다.
</details>
